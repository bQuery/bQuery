...
- **Server**: `rateLimit()` throttles incoming requests ([#223](https://github.com/bQuery/bQuery/issues/223)). No server-side throttling primitive existed, so any public endpoint was unprotected against brute force by default — including the login route in the `login-form` cookbook recipe, which now shows the limit. (`createRequestQueue` in `reactive` is the client-side mirror image: it limits outgoing parallel requests, not incoming ones.) Counters live in a `SessionStore`, the same abstraction sessions use, so a Redis-backed store plugs in the same way and the limit holds across processes. Responses carry `RateLimit-Limit`/`-Remaining`/`-Reset`, and a rejected request gets `429` with `Retry-After`. `keyBy` is required rather than defaulting to the client address: `ServerContext` exposes no peer address, so an implicit default could only read `X-Forwarded-For`, which a client sets itself — a limiter bypassed by varying a header is worse than none, because the app looks protected. `trustProxy: true` opts into reading it explicitly, and takes the **rightmost** entry of `X-Forwarded-For` — the hop the closest proxy appended, and therefore the only one it vouches for. No other forwarding header is consulted unless named (`trustProxy: 'cf-connecting-ip'`): a proxy that sets `CF-Connecting-IP` need not strip `X-Real-IP`, so falling back through a list of them would let a client choose its own bucket by sending a header the proxy never writes.
...
- **Toolchain**: the workflow branch guard no longer passes an unreadable trigger just because it has no branch list to check against ([#211](https://github.com/bQuery/bQuery/issues/211)). Without the upstream remote — on a fork, offline, in a shallow checkout — it exited 0 before looking at anything, discarding the parse problems it had already found along with the existence check that actually needs the list. An unterminated `branches: [` or a block it cannot read yields no refs, which is exactly the silent pass the guard exists to prevent, and finding it needs no network. Parse problems now fail the check on every path, under their own heading; only the existence check is skipped.

- **Server**: `serveStatic()` takes a precompressed sidecar's validators from the sidecar, and no longer offers ranges on its `304` ([#222](https://github.com/bQuery/bQuery/issues/222)). The encoded `ETag` and `Last-Modified` were derived from the identity file's size and mtime, so a `.br` or `.gz` rebuilt on its own — a stale sidecar replaced after a bad deploy — kept its old validators, and every cache holding the stale bytes revalidated them as fresh; `If-Modified-Since` compared against the identity file's date for the same reason. Separately, `Accept-Ranges` was dropped only after the `304` had already returned, so a revalidated compressed body advertised byte ranges its `200` had declined to offer, and caches merge a `304`'s headers into the stored entry.

- **Security**: character references now decode the way a browser's tokenizer does, on both backends ([#229](https://github.com/bQuery/bQuery/issues/229)). The decoder knew six names, lowercased them, and made the `;` optional for all of them, so `caf&eacute;` survived as literal text from the string backend and from the DOM backend's no-tags shortcut while `<b>caf&eacute;</b>` decoded through `DOMParser` — one input, two answers, depending on whether it contained a tag. It now reads names case-sensitively, requires `;` except for the 106 legacy names (which match as a prefix, longest first), maps `&#128;`–`&#159;` through windows-1252 and invalid code points to U+FFFD, and applies the attribute rule that leaves `href="?a=1&copy=2"` alone instead of turning it into `?a=1©=2`. The table covers every HTML 4.01 name, every name whose value is ASCII (`&colon;`, `&Tab;`, which matter to URL checks), and every name that shares a prefix with a legacy one; anything else stays literal and escaped. Checked against Chromium on 60,000 generated inputs in text and attribute context, with no differences outside names the table leaves out. The table costs about 1.8 kB gzipped in every entry that sanitizes, so the `core`, `component`, `security`, `view` and `storybook` budgets rise with it; the full HTML5 table would have cost about 7 kB.

- **Server**: limiters that share a `SessionStore` now serialize against each other ([#223](https://github.com/bQuery/bQuery/issues/223)). Each `rateLimit()` call built its own `serializeByKey` chain, so two limiters writing to one store — a login limiter and a global one, both keyed on the client address — never queued behind one another: concurrent requests read the same counter and wrote the same increment, making the effective ceiling `max × instances` for anyone opening parallel connections, which is the traffic shape a limiter exists to stop. The lock is now keyed on the store, in a `WeakMap`, so limiters with separate stores stay independent and a discarded store takes its chain with it. This still closes the single-process case only; a limit shared across processes needs a store with an atomic increment.

- **Security**: a trailing slash no longer closes an element that HTML will not let close itself ([#229](https://github.com/bQuery/bQuery/issues/229)). The DOM-free tokenizer treated `<script/>` as self-closing, which skipped raw-text consumption, so `stripTags('<script/>alert(1)</script>')` returned `alert(1)` — the same script-source-as-prose leak the shared text-extraction rule exists to prevent, reached through a different door. On an HTML element HTML honours the `/` only for void elements and drops it everywhere else, and so does the tokenizer now. Foreign content is the other case: inside `<svg>` and `<math>` every self-closing tag is acknowledged, `<path/>` included. The tokenizer honours it only on those two roots, which is enough because both are always-dropped tags whose whole subtree is removed, so how their descendants nest never reaches the output. Fixes `<template/>`, `<object/>`, `<noscript/>` and friends alongside `<script/>`.

- **Security**: entities decode on the DOM backend's text-only path ([#229](https://github.com/bQuery/bQuery/issues/229)). Input with no angle brackets bypasses `DOMParser` by design — it is what keeps plain text away from an HTML parser — but the `Text` node was built from the raw string, so entities survived as literal characters. `stripTags('Tom &amp; Jerry')` returned `Tom &amp; Jerry` where the string backend returned `Tom & Jerry`, and `sanitizeHtml()` escaped them a second time, turning `&lt;b&gt;` into `&amp;lt;b&amp;gt;`. Decoded with the shared `decodeEntities`, which is a pure string transform, so no text reaches a parser that did not before.

- **Security**: `sanitizeHtml(html, { stripAllTags: true })` now escapes its output ([#229](https://github.com/bQuery/bQuery/issues/229)). It returned the extracted text raw while branding it `SanitizedHtml`, a type whose whole purpose is "safe to assign to an HTML sink" — but the parser has already decoded entities by the time text is extracted, so `sanitizeHtml('<p>&lt;img src=x onerror=alert(1)&gt;</p>', { stripAllTags: true })` handed back live markup. The mutation-XSS fallback three lines away already escaped for exactly this reason. Both backends now do. `stripTags()` is unaffected and still returns raw plain text, which is what it documents; the two paths are no longer routed through one function, because they want opposite things.

- **Security**: text extraction drops the content of dangerous elements on both backends ([#229](https://github.com/bQuery/bQuery/issues/229)). The DOM backend returned `Node.textContent`, which includes the body of every `<script>` and `<style>` it passes, while the DOM-free backend suppressed those subtrees — so on **well-formed** input `stripTags('<script>alert(1)</script>')` gave `'alert(1)'` in the browser and `''` on the server, under the default `'auto'` backend, in the same app. Both now yield `''`. The rule lives in `sanitize-policy.ts` as `suppressesTextContent`, so it cannot drift again. Browser behaviour changes here: `stripTags()` and `stripAllTags` no longer surface script or style source as text.

- **Server**: `serveStatic` can revalidate a precompressed asset ([#222](https://github.com/bQuery/bQuery/issues/222)). `If-None-Match` was compared against the identity `ETag` while the response shipped the encoded one (`W/"…-br"`), so the two could never match and every conditional request for a `.br`/`.gz` sidecar re-sent the whole compressed body. The sidecar is now resolved before the conditional check, and the comparison uses the validator actually being sent. The `304` carries the encoded `ETag` and `Vary` but not `Content-Encoding`, which RFC 9110 §15.4.5 excludes from a `304`.

- **Server**: `serveStatic` honours `Accept-Encoding` quality values when choosing a sidecar ([#222](https://github.com/bQuery/bQuery/issues/222)). The qualities were parsed and used to reject `q=0`, but selection then walked a fixed brotli-then-gzip list, so `gzip;q=1.0, br;q=0.1` was served brotli. Candidates are now ranked by the client's stated preference, with ties keeping the previous order — so a plain `gzip, br` still prefers brotli.

- **SSR**: attribute and tag names are validated before being serialized ([#229](https://github.com/bQuery/bQuery/issues/229)). Both serializers escaped attribute _values_ and interpolated _names_ raw, while `readAttrName` and `readTagName` in the DOM-free parser end a name at whitespace, `=`, `>` and `/` — but not at a quote. `renderToString('<div a"b="c">')` therefore emitted `<div a"b="c">`, leaving what the markup means to the consumer's error recovery. Names outside `/^[a-zA-Z_:][a-zA-Z0-9_:.-]*$/` (tags: `/^[a-zA-Z][a-zA-Z0-9._:-]*$/`) are now dropped, and the shape is shared with the sanitizer rather than restated. Reachable through the DOM-free string backend, not the DOM one, which normalises such a name into two attributes. Output is unchanged for markup whose names match those patterns. The patterns are ASCII-only, so an attribute HTML itself permits but they do not — `<div é="x">` — is now dropped rather than serialized; this is a compatibility change for non-ASCII names.
