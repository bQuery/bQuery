...
- **Forms / Schema**: Added a fluent `schema({ name: field<string>().required().min(2), … })` helper that composes existing validator factories into a `FieldConfig` map.
...
- **Forms / SSR**: Added `serializeFormState(form)`, `readSerializedFormState(id)`, and `hydrateForm(form, snapshot)` helpers (built on `src/ssr/escape.ts`) so server-rendered form state can resume on the client.
