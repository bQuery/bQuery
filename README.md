...
<a href="https://bundlephobia.com/package/@bquery/bquery"><img src="https://img.shields.io/bundlephobia/minzip/@bquery/bquery?style=flat" alt="Bundle Size (Bundlephobia)"/></a>&nbsp;
<a href="https://bundlejs.com/?q=%40bquery%2Fbquery"><img src="https://deno.bundlejs.com/?q=@bquery/bquery&badge=simple&style=flat" alt="Bundle Size (BundleJS)"/></a>&nbsp;
...
> (the single source of truth, enforced by `bun run check:stability`). **As of
> 1.15.0 every module is Stable** — the release graduated the final thirteen
> (`view`, `forms`, `i18n`, `a11y`, `dnd`, `media`, `plugin`, `devtools`,
> `testing`, `storybook`, `concurrency`, `ssr`, `server`).
...
| **Concurrency** | Stable | Zero-build worker tasks, explicit RPC helpers, optional reactive state wrappers, bounded worker pools, high-level collection helpers, an optional fluent pipeline layer, CSP-safe module workers, and client UI-scheduling primitives |
| **Component**   | Stable | Typed Web Components with scoped reactivity and configurable Shadow DOM                                                                                                                                                               |
| **Storybook**   | Stable | Secure-by-default story template helpers with a pinned `unsafeHtml` contract                                                                                                                                                          |
...
| **Router**      | Stable | SPA routing, constrained params, redirects, guards, `useRoute()`, `<bq-link>`, and an opt-in file-route convention with typed `load` / `action`                                                                                       |
| **Store**       | Stable | Signal-based state management, persistence, migrations, action hooks, and plugin lifecycle helpers                                                                                                                                    |
| **View**        | Stable | Declarative DOM bindings with `bq-*` directives, declarative enter/leave/move transitions, and an optional expression precompiler                                                                                                     |
| **Forms**       | Stable | Reactive form state, validation, field arrays, progressive-enhancement form actions, and optimistic updates                                                                                                                           |
| **i18n**        | Stable | Reactive locales, interpolation, ICU MessageFormat plurals, lazy loading, Intl formatting, and optional message-extraction tooling                                                                                                    |
| **A11y**        | Stable | Focus traps, live-region announcements, roving tabindex, skip links, and audits with documented WCAG scope                                                                                                                            |
