...
- **1.15.0** — **graduated to Stable**: surface frozen for one minor cycle ([#142](https://github.com/bQuery/bQuery/issues/142)). Audit scope documented with WCAG mappings and known limitations; new `auditRules` catalog and a `wcag` field on every `AuditFinding`. Focus/live-region/`inert`/`scrollLock`/preference behaviour covered by tests.
