...
//    `await` needs an async context in CommonJS; top-level `await` is a
//    SyntaxError there.
async function main() {
...
//    Browser and reactive APIs only — see the caveat below.
...
::: warning Option 2 cannot run server or SSR code
The UMD bundle stubs Node built-ins, so `server` and `ssr` APIs are present
on the namespace but throw when used:

```js
const b = require('@bquery/bquery');
b.createServer().listen(0);
// Error: [bQuery] "node:http" is not available in the browser bundle.
```

For those APIs use option 1 or 3. That is also why the `✅ UMD` cells above
mean "loads", not "everything in it works on a server runtime".
:::

`./package.json` is exported too, for tooling that resolves it (several bundler plugins and type-resolution tools expect to).

#### Types follow the same split

Each entry declares its `types` _inside_ the matching condition, and the
`require` branches point at a `.d.cts` tree emitted next to the ESM one:

```json
".": {
  "import": { "types": "./dist/index.d.ts", "default": "./dist/index.es.mjs" },
  "require": { "types": "./dist/index.d.cts", "default": "./dist/full.umd.cjs" }
}
```

A single shared `types` would resolve a CommonJS consumer's types to the ESM
declaration — which, in a `"type": "module"` package, TypeScript rejects with
`TS1479` under `module: node16`, even though the runtime resolution is fine.
publint and `attw` both report that as FalseESM. The `.d.cts` tree is
generated by `scripts/postbuild-cts.mjs` during `build:types`.
