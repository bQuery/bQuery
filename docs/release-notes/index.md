...
| ------------------------------- | ----------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [1.17.0](/release-notes/1.17)   | 2026-09-21              | `server` gains `serveStatic()` and `rateLimit()`; `sanitizeHtml()` works without a DOM; opt-in glitch-free reactive scheduler plus a flush-ordering fix that applies to `batch()` by default. Packaging: `require('@bquery/bquery')` works, sub-path types resolve, JS source maps dropped. One path break — `dist/full.umd.js` is now `dist/full.umd.cjs`. |
| [1.16.1](/release-notes/1.16.1) | 2026-08-26              | Toolchain-and-build maintenance patch: Bun floor moves to `1.4.0`, dev dependencies refreshed, Vite configs build warning-free (`import.meta.dirname`, `node:http` stub). No source or API changes.                                                                                                                                                         |
