...
**Two things to know.** On malformed input the backends can differ, always with the string one more conservative: `<div><scr<script>ipt>alert(1)</script></div>` gives `<div></div>` under DOM and `<div>ipt&gt;alert(1)</div>` under string — inert either way, one as nothing and one as escaped text. Pin `backend: 'string'` if you need identical output everywhere.
