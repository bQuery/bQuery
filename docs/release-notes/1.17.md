...
**`keyBy` is required, and that is deliberate.** `ServerContext` exposes no peer address, so the only available "client address" is `X-Forwarded-For` — which the _client_ sets unless a proxy you control overwrites it. Defaulting to it would ship a limiter that an attacker bypasses by varying a header, which is worse than no limiter because the app looks protected. `trustProxy: true` opts into the forwarded-header behaviour explicitly.
