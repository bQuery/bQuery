...
4. **Run the local validation suite**: `bun run lint`, `bun run format`, `bun run lint:types`, `bun run test:types`, `bun test`, `bun run check`, and `bun run build`. The PR gate runs the same set — the `Test` workflow's `quality` job (`lint:types`, `eslint .`, `format:check`, `check`) and its `test` job (`test:types`, `bun test`), plus `bun run build` in the `SSR cross-runtime` workflow — so anything failing there fails locally first. `test:types` matters: the root `tsconfig.json` excludes `tests/`, so a type error in a test file passes `lint:types` and only surfaces in CI.
