...
export const BUNDLE_BUDGETS = [
  ...
  { subpath: './full', gzip: 148800, note: 'Every public export; for CDN use, not bundlers.' },
  {
    subpath: './core',
    gzip: 18600,
    note: 'Sanitizes html() writes, so it carries the sanitizer and its character-reference table, which it needs to decode like a browser (+1.8 kB, 1.17.0).',
  },
  ...
  { subpath: './concurrency', gzip: 11100 },
  {
    subpath: './component',
    gzip: 17200,
    note: 'Sanitizes its own DOM writes, so it carries the security module (#229) and its character-reference table (1.17.0).',
  },
  { subpath: './motion', gzip: 12600 },
  {
    subpath: './security',
    gzip: 7600,
    note: 'Ships both sanitizer backends — the DOM one and the DOM-free string scanner (#229) — because which one runs is decided at runtime, not at bundle time. About 1.3 kB of it is the HTML 4.01 character-reference table both backends decode with (1.17.0).',
  },
  ...
  { subpath: './store', gzip: 5100 },
  {
    subpath: './view',
    gzip: 20200,
    note: 'Sanitizes bq-html, so it carries the sanitizer and its character-reference table, which it needs to decode like a browser (+1.8 kB, 1.17.0).',
  },
  { subpath: './view/compiler', gzip: 5700 },
  {
    subpath: './storybook',
    gzip: 8600,
    note: 'Re-exports the sanitizer for unsafeHtml(), so it grew with #229 and again with its character-reference table (1.17.0).',
  },
